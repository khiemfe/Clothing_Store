        ...
        <MDBContainer className="my-5 sign" onKeyDown={handleKeyDown}>
          <MDBCard>
            <MDBRow className='g-0'>
              <MDBCol md='6'>
                <MDBCardImage src='https://binbadecor.vn/wp-content/uploads/2022/03/thiet-ke-shop-quan-ao-2.jpg' alt="login form" className='rounded-start w-100 image'/>
              </MDBCol>
              <MDBCol md='6'>
                <MDBCardBody className='d-flex flex-column'>
                  ...
                  <h2 className="fw-normal my-4 pb-3 title" style={{letterSpacing: '1px', textAlign: 'center'}}>ĐĂNG KÝ</h2>
                    {/* <MDBInput wrapperClass='mb-4' label='Name' id='formControlLg' type='text' size="lg"/> */}
                    <div className='input'>
                      <h5 className="text">Email:</h5>
                      <MDBInput className='input-nhap' wrapperClass='mb-4' onChange={handleOnChangeEmail} id='formControlLg' type='email' size="lg"/>
                    </div>
                    <div className='input'>
                      <h5 className="text">Password:</h5>
                      <MDBInput className='input-nhap' wrapperClass='mb-4' onChange={handleOnChangePassword} id='formControlLg' type='password' size="lg"/>
                    </div>
                    <div className='input'>
                      <h5 className="text">Cofirm Password:</h5>
                      <MDBInput className='input-nhap' wrapperClass='mb-4' onChange={handleOnChangeConfirmPassword} id='formControlLg' type='password' size="lg"/>
                    </div>
                  <div className='footer'>
                    <div className="more">
                      { data?.status === 'ERR' && <span className='err' style={{color: 'red'}}>{data?.message}</span> }
                    </div>
                    <div className="button" style={{marginTop: '15px', display: 'flex', alignItems: 'center'}}>
                        <div style={{marginRight: '20px'}}>
                          <LoadingComponents isLoading={isLoading} />
                        </div>
                        <Button onClick={handleSignUp} className='btn-login'
                          style={{backgroundColor: '#000', cursor: 'pointer', border: 'none', borderRadius: '8px'}}>
                            SIGN UP
                        </Button>
                    </div>
                  </div>
                  <div className='qmk-dk'>
                    <a href="/sign-in" style={{color: '#393f81'}}>Đăng nhập</a>
                  </div>
                </MDBCardBody>
              </MDBCol>
            </MDBRow>
